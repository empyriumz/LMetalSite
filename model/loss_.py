...
class AUCMLoss_V2(torch.nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        loss = (
            ...
            * (
                self.margin
                + self.mean(y_pred * neg_mask)
                - self.mean(y_pred * pos_mask)
            )
            - self.alpha**2
        )
        return loss
...
class AUCM_MultiLabel_V2(torch.nn.Module):
    ...
    def __init__(self, margin=1.0, num_classes=10, device=None):
        ...
        self.a = torch.zeros(
            num_classes, dtype=torch.float32, device=self.device, requires_grad=True
        ).to(self.device)
        self.b = torch.zeros(
            num_classes, dtype=torch.float32, device=self.device, requires_grad=True
        ).to(self.device)
    ...
    def forward(self, y_pred, y_true):
        ...
        for idx in range(self.num_classes):
            ...
            loss = (
                self.mean((y_pred_i - self.a[idx]) ** 2 * pos_mask)
                + self.mean((y_pred_i - self.b[idx]) ** 2 * neg_mask)
                + 2
                * self.alpha[idx]
                * (
                    self.margin
                    + self.mean(y_pred_i * neg_mask)
                    - self.mean(y_pred_i * pos_mask)
                )
                - self.alpha[idx] ** 2
            )
            total_loss += loss
...
class APLoss(torch.nn.Module):
    ...
    def __init__(
        ...
        if not device:
            ...
            self.device = device
        self.u_all = (
            torch.tensor([0.0] * pos_len).reshape(-1, 1).to(self.device).detach()
        )
        self.u_pos = (
            torch.tensor([0.0] * pos_len).reshape(-1, 1).to(self.device).detach()
        )
    ...
    def forward(self, y_pred, y_true, index_p):
        ...
        self.u_all[index_p] = (1 - self.gamma) * self.u_all[index_p] + self.gamma * (
            sur_loss.mean(1, keepdim=True)
        ).detach_()
        self.u_pos[index_p] = (1 - self.gamma) * self.u_pos[index_p] + self.gamma * (
            pos_sur_loss.mean(1, keepdim=True)
        ).detach_()
        p = (self.u_pos[index_p] - (self.u_all[index_p]) * pos_mask) / (
            self.u_all[index_p] ** 2
...
class pAUC_CVaR_Loss(torch.nn.Module):
    ...
    def forward(self, y_pred, y_true, index_p, auto=True):
        ...
        loss = self.surrogate_loss(
            self.margin, f_ps - f_ns.transpose(0, 1)
        )  # return element-wsie loss
        p = loss > self.u_pos[index_p]
        self.u_pos[index_p] = self.u_pos[index_p] - self.eta / self.pos_len * (
            1 - p.sum(dim=1, keepdim=True) / (self.beta * self.num_neg)
        )
        p.detach_()
...
class pAUC_DRO_Loss(torch.nn.Module):
    ...
    def forward(self, y_pred, y_true, index_p):
        ...
        self.u_pos[index_p] = (1 - self.gamma) * self.u_pos[index_p] + self.gamma * (
            exp_loss.mean(1, keepdim=True).detach()
        )
...
class tpAUC_KL_Loss(torch.nn.Module):
    ...
    def forward(self, y_pred, y_true, index_p):
        ...
        self.u_pos[index_p] = (1 - self.gamma0) * self.u_pos[index_p] + self.gamma0 * (
            exp_loss.mean(1, keepdim=True)
        )
        self.w = (1 - self.gamma1) * self.w + self.gamma1 * (
            torch.pow(self.u_pos[index_p], self.Lambda / self.tau).mean()
        )
        p = (
            torch.pow(self.u_pos[index_p], self.Lambda / self.tau - 1)
            * exp_loss
            / self.w
        )
        p.detach_()
