...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def _buffered_future_mask(self, tensor, tensor2=None):
        dim1 = dim2 = tensor.size(0)
        if tensor2 is not None:
            dim2 = tensor2.size(0)
        future_mask = torch.triu(
            self._fill_with_neg_inf(torch.ones(dim1, dim2, device=tensor.device)),
            1 + abs(dim2 - dim1),
        )
    ...
    def forward(self, x, x_k=None, x_v=None):
        ...
        mask = self._buffered_future_mask(x, x_k) if self.attn_mask else None
